...
import { Address,
    ...
    hash_script_data,
    hash_plutus_data,
    ScriptDataHash, Ed25519KeyHash, NativeScript, StakeCredential } from '@emurgo/cardano-serialization-lib-asmjs';
 ...
 * @returns {Nft[]}
 */
export const fetchNftData = (nfts): Nft[] => {
    const newNfts = [...nfts];
    newNfts.forEach(nft => {
        const handleAsync = async () => {
            const nft_data = await getAssetData(nft.asset);
            nft.data = nft_data;
        }
...
export const getPlutusContractData = async (address) => {
 ...
 * @returns {Array}
 */
export const fetchPlutusContractNftData = async (props) => {
    ...
    if (data) {
        let assets = data.amount.slice(0, 100);
        assets.forEach(nft => {
            const handleAsync = async () => {
    ...
    props.dispatch(actions.saveWallet({plutusNfts: nft_array}));
    return nft_array;
}
 ...
 */
export const enableWallet = async (props): Promise<boolean> => {
...
export const disableWallet = (props) => {
    API = undefined;
...
/**
     ...
     * @returns {boolean}
     */
export const checkIfWalletFound = (props): boolean => {
    const walletKey = props.whichWalletSelected;
    const walletFound = !!window?.cardano?.[walletKey];
    props.dispatch(actions.saveWallet({walletFound: walletFound}));
 ...
 * @returns {Promise<boolean>}
 */
export const checkIfWalletEnabled = async (props): Promise<boolean> => {
    let walletIsEnabled = false;
    const walletName = props.whichWalletSelected;
    walletIsEnabled = await window.cardano[walletName].isEnabled();
    props.dispatch(actions.saveWallet({walletIsEnabled: walletIsEnabled}));
 ...
 */
export const getBalance = async (props): Promise<number> => {
    try {
        const balanceCBORHex = await API.getBalance();
        const balance = Value.from_bytes(Buffer.from(balanceCBORHex, "hex")).coin().to_str();
        props.dispatch(actions.saveWallet({balance: balance}));
        return balance;
    } catch (err) {
        console.log(err);
    }
 ...
 * @returns {Promise<void>}
 */
export const getNetworkId = async (props): Promise<number> => {
    try {
        const networkId = await API.getNetworkId();
        props.dispatch(actions.saveWallet({networkId: networkId}));
        return networkId;
    } catch (err) {
        console.log(err);
    }
 ...
 * @returns {*}
 */
export const getAPIVersion = (props): string => {
    const walletKey = props.whichWalletSelected;
    const walletAPIVersion = window?.cardano?.[walletKey].apiVersion;
    props.dispatch(actions.saveWallet({walletAPIVersion: walletAPIVersion}));
 ...
 * @returns {*}
 */
export const getWalletName = (props): string => {
    const walletKey = props.whichWalletSelected;
    const walletName = window?.cardano?.[walletKey].name;
    props.dispatch(actions.saveWallet({walletName: walletName}));
    return walletName;
}
 ...
 * @returns {Promise<void>}
 */
export const getCollateral = async (props): Promise<any[]> => {
    let CollatUtxos = [];
    ...
    try {
        ...
        props.dispatch(actions.saveWallet({CollatUtxos: CollatUtxos}));
 ...
 * @returns {Promise<void>}
 */
export const getChangeAddress = async (props): Promise<string> => {
    try {
        const raw = await API.getChangeAddress();
        const changeAddress = Address.from_bytes(Buffer.from(raw, "hex")).to_bech32();
        props.dispatch(actions.saveWallet({changeAddress: changeAddress}));
 ...
 * @returns {Promise<void>}
 */
export const getRewardAddresses = async (props): Promise<string>  => {
    try {
        const raw = await API.getRewardAddresses();
        const rawFirst = raw[0];
        const rewardAddress = Address.from_bytes(Buffer.from(rawFirst, "hex")).to_bech32();
        props.dispatch(actions.saveWallet({rewardAddress: rewardAddress}));
 ...
 * @returns {Promise<void>}
 */
export const getUsedAddresses = async (props): Promise<string> => {
    try {
        let usedAddress = '';
        ...
        props.dispatch(actions.saveWallet({usedAddress: usedAddress}));
        return usedAddress;
    } catch (err) {
        console.log(err);
    }
 ...
 * @returns {Promise<void>}
 */
export const getUtxos = async (props): Promise<{Utxos: Utxo[], Nfts: Nft[]}> => {
    let Utxos = [];
    let Nfts = [];
    ...
    try {
        ...
        props.dispatch(actions.saveWallet({Utxos: Utxos, Nfts: Nfts}));
        return { Utxos: Utxos, Nfts: Nfts }
    } catch (err) {
        console.log(err);
    }
...
* @returns {Promise<void>}
*/
export const refreshData = async (props): Promise<Object> => {
    try{
        ...
        if (walletFound) {
            ...
            if (walletEnabled) {
                ...
                const nfts = fetchNftData(utxosAndNfts.Nfts);
 ...
 */
export const initTransactionBuilder = async (protocolParams) => {
 ...
 */
export const getTxUnspentOutputs = async (props) => {
...
export const buildSendTokenToPlutusScript = async (nftKey, state) => {
    debugger;
    ...
    state.dispatch(actions.saveWallet({submittedTxHash: submittedTxHash, transactionIdLocked: submittedTxHash, lovelaceLocked: state.lovelaceToSend}));
}

export const buildRedeemTokenFromPlutusScript = async (asset_name, policy_id, state) => {
    const txBuilder = await initTransactionBuilder(state.protocolParams);
    const ScriptAddress = Address.from_bech32(state.addressScriptBech32);
    const shelleyChangeAddress = Address.from_bech32(state.changeAddress);
    const assetNameHex = '5a656c6356656e64696e674d616368696e6530303735';//Buffer.from(asset_name, "utf8").toString("hex");
    ...
    state.dispatch(actions.saveWallet({ submittedTxHash: submittedTxHash }));
}